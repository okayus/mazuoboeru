...
- **リポ構成**: pnpm workspaces（globs: `apps/*` `packages/*`）、パッケージ名 `@mazuoboeru/*`。
  ...
  - **コンテナ内 claude の既定モデルは Fable 5**（compose の `ANTHROPIC_MODEL=claude-fable-5`、2026-06-12）。**`/model` ピッカーには Fable が出ない**: 一覧は statsig の feature flag 依存で、本サンドボックスは statsig を意図的に遮断しているため（telemetry 方針）。API 自体は Fable を受けるので env/`--model` 指定は有効、`/model` でのセッション内切替も従来どおり。ピッカーに出したければ statsig 許可が必要だがトレードオフ＝未採用。
