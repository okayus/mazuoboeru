...
- **タグはフラット**（2026-08-21・[ADR-0016](docs/adr/0016-flat-tags-related-tags-derived-from-quizzes.md)）: タグ間の関係は**保存しない**（ADR-0007 の `tag_edge` タクソノミは撤回・DROP）。関連は **Related Tags**＝公開クイズに一緒に付いている事実（共起）から読み時に導出（純粋関数は `packages/core`）。絞り込みは `?tags=a&tags=b` の AND 完全一致。
