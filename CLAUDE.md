...
- **リポ構成**: pnpm workspaces（globs: `apps/*` `packages/*`）、パッケージ名 `@mazuoboeru/*`。
  ...
  - **コンテナ内 claude の既定モデルは Fable 5**（compose の `ANTHROPIC_MODEL=claude-fable-5`、2026-06-12）。`/model` ピッカーの品揃えは statsig の feature flag 依存。同日 statsig を **optional 許可**し（解決失敗しても起動継続）、env を個別フラグへ分解して **Statsig のみ解除**＝ピッカーにも Fable が出る。sentry・自動更新・/feedback は引き続き無効（`DISABLE_ERROR_REPORTING` / `DISABLE_AUTOUPDATER` / `DISABLE_FEEDBACK_COMMAND`、更新は起動時 npm のみ）。
- **Claude Code ツールセット**（2026-06-08 整備）:
  ...
  - サンドボックス firewall に `developers.cloudflare.com` と `docs.mcp.cloudflare.com` を追加（`.docker/init-firewall.sh`）。`statsig.anthropic.com` は **optional 扱い**で追加（2026-06-12。feature flag 用。解決失敗は警告のみでコンテナは起動継続）。**反映には `docker compose down && docker compose build && docker compose up -d`（プロジェクトディレクトリで、`-f` を付けず＝override 自動ロード）が必要**。
