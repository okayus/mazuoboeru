...
7. **エージェント**: ホスト側リレー（push/PR 代行）構築 → 無人 E2E（コンテナ commit → 自動 push → PR → CI green → merge → Workers Builds → 本番 `/health` 200）。RP_ID/ORIGIN の本番固定はデプロイ確認後。

## 日常運用

- 起動: `docker compose up -d` ／ シェル: `docker compose exec dev zsh` ／ 停止: `docker compose stop`。
- git 運用（[ADR-0003](adr/0003-secrets-strategy.md) 確定形）: **commit はコンテナ内可**（秘密不要）、**push/PR はホスト側リレーが代行**（GitHub App・`claude/*` ブランチのみ）。リレー稼働までの移行期間は従来どおり commit/push ともホスト側（`.claude/settings.json` の deny 緩和もリレー稼働後）。
