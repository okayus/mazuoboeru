...
## 補記（2026-09-13）— Challenge Result（挑戦結果）は画面上の導出だけで復活

本 ADR が「失われるもの」に挙げた **per-run スコア表示（1回の通しで 7/10）** を、[[CONTEXT.md]] の **Challenge Result（挑戦結果）** として復活させる。ただし**画面上の導出だけ**——クイズ単位 [[Drill]] の1回の通しで出した [[Answer]] の正誤をクライアントが手元で束ね、全設問を解き終わった画面でのみ「N / M 問正解」と設問ごとの正誤・正解・（不正解なら）自分の答え・解説を表示する。**通しの括りを表す行も、結果の保存も持たない**（離脱・再読み込みで消える）ので、本 ADR の決定——Attempt の引退・単一フラット `answer`・進行状態なし・resume なし——は不変。サーバ・DTO・スキーマの変更もない。保存が要る（過去の通しを見返したい）と分かった時点で、それは本 ADR を一部巻き戻す別の決定として ADR を起こす。対象はクイズ単位の通しだけで、[[Review List]] Drill の終了画面（「覚えた」で外した数）は変えない。
