...
>
> **最終更新: 2026-07-06**（更新したら日付と §「いま動いているもの」を直す）
...
- **Attempt エンティティを引退・回答を単一 `answer` に統合（本番反映 2026-06-24・#69＝main `a0918512`・[ADR-0013](adr/0013-retire-attempt-unify-answers.md)）**: 「1クイズ1回の記録（完了・再開・7/10 スコア）」を持つ [[Attempt]] を廃し、回答を**単一フラット `answer(id,user_id,question_id,is_correct,answered_at)`** に統合（旧 `review_answer` をリネーム＝migration 0009＋旧 `attempt_answer` を移行＝0010・既存データ無損失）。**「挑戦」は UI 動詞**として残し＝**クイズ単位の [[Drill]]**（1問ずつ・サーバー採点 `gradeQuestion`・即時フィードバック・設問順&選択肢シャッフル・☆復習リストトグル／per-run スコアと resume は廃止）。`#/quiz/:id` を `QuizDrill`（共有カード `DrillQuestionCard` を Review List ドリルと共有）に置換、`Challenge.tsx`/attempts ルート/`decideAnswer` を撤去。ダッシュボードを**単一 `answer` に一本化＋クイズ別正答率の軸**を追加（純関数 `bundleQuizAccuracy`・意味論は不変＝[ADR-0006](adr/0006-dashboard-aggregation-semantics.md) addendum）。**退行ゼロ順序**＝代替（クイズ単位 Drill）を先に出してから旧経路を撤去したので golden-path e2e は全スライス緑。実装は**3スライス**（#69 で squash・relay がブランチ削除＝再 merge ループ無し）。**本番反映の確認**: host が本番エクスポートの実データで dry-run（`answer`=124＝review_answer24+attempt_answer100・無損失・`foreign_key_check` clean・ID 衝突なし）＋`wrangler d1 export` backup → #69 マージで **`Workers Builds: mazuoboeru` success＝`d1 migrations apply --remote && deploy`（0009→0010 適用）** → `/health` ok・SPA が `index-B6BM-27-.js`（ローカル build と一致）を配信 を確認（本番 `answer` 実数は host の `--remote` SELECT で最終確認可）。tsc(web/e2e)・unit 91・lint・fmt・build・e2e 6/6 も green。data-model.md 改訂・CONTEXT.md は前 grill で Challenge/Answer/Drill/Streak を改訂済み。
- **ホットリスト単問ダイアログ（2026-06-28・PR #70＝main `5eddc48`・本番反映・実機確認済み）**: my hot list の各設問に「**解く**」を追加し、その1問だけをダイアログ（ネイティブ `<dialog>`）で解いて即時フィードバック（正誤・正解・解説・マークダウンコピー）まで表示する。ホットリストから離脱しない。「出典」は引用元クイズへのリンクのまま据え置き。回答後は「覚えた（外す）」で復習リストから除外して閉じる／「閉じる（まだ）」で残す。grill 確定＝新概念なし＝既存 Drill の「プール1問版」として実装（CONTEXT.md の Drill を一行更新）。バックエンドは読み取り1本 `GET /api/drill/question/:questionId` 追加（`is_correct` 伏せ・公開ゲート・session 限定）、採点は既存 `POST /api/drill/answers` 無改変流用。スキーマ変更・マイグレーション・ADR なし。回答は通常の `Answer` なのでダッシュボード自動算入。検証：tsc/lint/fmt/build 全緑、unit 97 passed、使い捨て Playwright で実ブラウザ検証（作成→公開→☆追加→解く→サーバー採点→覚えた），既存 e2e 6/6 緑（回帰なし）。本番確認：Workers Builds success・`/health` 200・新エンドポイント 401（session ガード）・SPA バンドル `index-BpC06qSJ.js` 本番配信。
- **その先の実装候補（次々セッション以降）**: ①Phase 2 残〔人気/ランキング・作者ページ・追加設問形式 `short`（実装・ローカル検証済＝ADR-0012・本番反映前）→`boolean`→`cloze`・Passkey・通報の Discord 通知・CLI npm 配信〕／②Phase 3 残〔タグ別習熟度（`answer` の蓄積を実効タグで集計）〕／③ツールチェイン段2〔dev/build の Rolldown 化＝wrangler 3→4＋@cloudflare/vite-plugin 1.x＋`vp build`/`vp dev`＋ratelimit 正式形移行。段1（lint/fmt/CI）は #63 反映済み＝[ADR-0009](adr/0009-vite-plus-toolchain.md)〕／④Phase 1 残〔投稿 per-user レート制限＝一般公開前・当面 allowlist で代替〕。⑤Attempt の引退は **#69 で本番反映済み**（main `a0918512`・上記エントリ参照）。⑥**公開後編集の実装ギャップ解消ほか、クイズ削除・公開後編集ライフサイクルの再設計**（§「ここから必要なもの」D・2026-07-06 調査）。
...
| **ホットリスト単問ダイアログ（#70）** | ✅ **稼働（2026-06-28 本番確認）** | my hot list の各行に「解く」を追加＝その1問だけをダイアログで解く・ホットリストから離脱しない。ダイアログ内で回答→サーバー採点 `POST /api/drill/answers`（無改変・既存）→即時フィードバック表示。新エンドポイント `GET /api/drill/question/:questionId` **401**（session 限定・`is_correct` 伏せ・公開ゲート）。「出典」は引用元クイズへのリンク据え置き。スキーマ変更・マイグレーション・ADR なし。回答は通常の `Answer` なのでダッシュボード自動算入。本番 SPA `index-BpC06qSJ.js` 配信・Workers Builds success・既存 e2e 回帰なし |

> つまり **データ層・公開読み取り・採点・セキュリティ境界・ログイン入口まで本番で生きている**。Phase 1 縦切りは人間がブラウザで端から端まで使える状態。**Phase 2（発見と振り返り）の主要機能も本番で稼働**（タグ検索・ダッシュボード・my hot・1画面1問の挑戦・単問ダイアログ）。
...
### D. クイズ削除・公開後編集のライフサイクル（2026-07-06 調査・consumer 側の実害起点）

**起点**: クイズ登録ワークスペース（`../mazuoboeru-quizzes/`）で、公開済みクイズの設問1問の欠陥（設問間参照）を直す手段がなく「**ソフト削除→再登録**」で対処した（旧 `55347c50-…` → 新 `9234d2ea-…`）。波及: ①review_list の該当設問は行が残るのに一覧・ドリルから黙って落ちる（read 時に published＋非削除で JOIN フィルタ＝`review-list-queries.ts` の設計どおりだが、復習途中の文脈が無警告で消える）②旧回答はダッシュボードに実タイトルのまま集計され続け、per-quiz 軸に旧・新クイズが二重に並ぶ。

**用語と評価**: ソフト削除＝論理削除（`deleted_at`）。論理削除アンチパターン論（クエリ汚染・UNIQUE 阻害・「削除」の意味の曖昧さ）のうち本アプリに実際に当たるのは、(a) 公開述語 `status='published' AND deleted_at IS NULL` を 12 箇所＋規約で維持する運用コスト（2026-07-06 の全数調査で漏れ無し。ただし `loadQuizWithContent`〔`worker/db/quiz-queries.ts`〕は無フィルタの生ローダで、新規呼び出し側が deleted_at チェックを忘れると露出する footgun）、(b) 「削除／hidden／self-hide」の意味分散（[CONTEXT.md](../CONTEXT.md) に削除の用語エントリが無い）。UNIQUE 阻害は quiz に UNIQUE 制約が無く非該当。

**対応方針（優先順）**:

1. **［根治・最優先］公開後編集の実装ギャップ解消**: [ADR-0002](adr/0002-publish-flow-and-edit-rules.md) は「公開後の編集は軽微（タイトル・説明・**設問文・解説**）は自由／重大（正解変更・選択肢の追加削除）も **UI 警告のみで構造制約はかけず**サーバは受理／過去の `answer.is_correct` は再計算しない（履歴の自立性＝[ADR-0010](adr/0010-server-side-grading-rationale.md)）」と**既決**。ところが実装は published への `questions` 変更を一律 **409 `cannot_restructure_published`**（`worker/routes/quizzes.ts` の PATCH は published では metaSchema＝タイトル・説明のみ受理、content 置換は draft 限定の `replaceDraftContent`）で拒否しており **ADR より厳しい**。設問単体の編集 API も無い。→ **2026-07-06 grill で設計確定（[ADR-0014](adr/0014-published-quiz-editing-and-question-retirement.md)・[CONTEXT.md](../CONTEXT.md) Retired・ADR-0002 追記）**: ①設問の削除＝物理 DELETE でも論理削除フラグでもなく**状態遷移 `question.status: active → retired`（不可逆）**（answer/review_list の NO ACTION FK と履歴の自立性が根拠。migration 0011＝純粋 ADD COLUMN・rebuild 無し）②API＝**フル文書 PATCH の diff-apply**（id あり=UPDATE・type 変更拒否／id 欠落=retire／id 無し=INSERT＝公開後の設問追加もこれ／未知 id=400／配列順=position／応答に diff サマリ `{updated,added,retired}`）③編集ゲート＝**適用後 active 集合へ `validateForPublish` を再利用**（構造の破壊は 422 拒否・意味的重大編集は受理＝「採点可能である限り受理」）④retire 不可逆・作者 GET は active のみ⑤スライス=**サーバ＋CLI `mzo update`**（README doc-drift 解消込み。**Web 編集 UI は後回し**＝ブラウザ作成は未使用のため実装候補⑥へ）。`quiz_revision` は前提にしない（ADR-0002 が「MVP の重荷にしない」と明言済み・Phase 2 候補のまま）。実装後は consumer 側（`../mazuoboeru-quizzes/` の CLAUDE.md／create-quiz skill の「公開後はタイトル・説明・タグのみ」）を追従させる（リポ外＝別作業）。
2. **物理削除は解ではない（現状維持）**: `answer.question_id` は FK **NO ACTION** ＝ 回答が存在する限り question/quiz の物理 DELETE は FK 違反で不可能だし、履歴ごと消すのは「履歴の自立性」（ADR-0010／[ADR-0013](adr/0013-retire-attempt-unify-answers.md)）と正面衝突する。ハード削除は Phase 4 のアカウント削除（GDPR）文脈に集約のままでよい（[ADR-0002](adr/0002-publish-flow-and-edit-rules.md) の判断は現行モデルでも有効）。
3. **削除の状態化は「次に quiz を rebuild する migration」に相乗り**: `quiz.status` は既に 3 値の状態機械（`draft|published|hidden`・hidden への書き込み経路は未実装）で、`deleted_at` はその外の第 2 軸＝二重管理がクエリ汚染の源。`deleted`（or `retracted`）を status に統合して公開述語を `status='published'` 1 本に畳み、`deleted_at` は監査時刻へ格下げるのが筋。ただし D1 の CHECK 変更＝quiz の table rebuild（0008 の子 repoint 手順・スキル `cloudflare-d1-drizzle-migration`）なので**単独 migration では割に合わない**＝次に quiz を rebuild する機会に相乗りする。着手時に ADR-0002 へ追記（ソフト削除根拠の語彙を「他人の attempt 履歴が孤立」→「answer 履歴の自立性」へ更新）＋ CONTEXT.md へ削除／ハード削除の用語を追加。
4. **独立の小修正**: ダッシュボード per-quiz 軸の「（削除済みのクイズ）」プレースホルダが**論理削除では発火しない**（`worker/db/dashboard-queries.ts` の `quizTitlesByIds` が `deleted_at` を見ない＝行が消える物理削除でしか undefined にならない。プレースホルダは `worker/routes/dashboard.ts`）。ラベルの意図と実挙動を一致させる（deleted_at を見てプレースホルダへ置換するか、実タイトル表示を仕様として明記）。
...
- ~~**Attempt の引退（ステートレス化の最終形）**~~ → ✅ **本番反映済み（2026-06-24・#69＝main `a0918512`・[ADR-0013](adr/0013-retire-attempt-unify-answers.md)）**: [[Attempt]] エンティティを引退し回答を単一フラット `answer` に統合・「挑戦」をクイズ単位 [[Drill]] 化・ダッシュボードにクイズ別軸（退行ゼロ順序・3スライス・host dry-run＋backup→Workers Builds が 0009→0010 を本番適用）。詳細は上記『直近の前進』のエントリ。auto-memory `attempt-to-become-stateless-future`。
...
- **`apps/cli/README.md` が #67（short 追加）に未追従＝doc-drift（実害は低・要更新／2026-06-27 把握）**: README §「入力 JSON」が `type` を **`mcq_single` / `mcq_multi` のみ**と列挙し例も mcq だけ＝**`short` 型と `answer`（許容解）フィールドが未記載**（[ADR-0012](adr/0012-short-answer-grading-normalization.md)・#67 で追加済み）。正典は実コードの zod（`apps/web/worker/routes/quizzes.ts`＝`type` enum に `short`／`answer: string[]`〔≤10・各≤200・`MAX_ACCEPTED_ANSWERS`/`MAX_ANSWER_LEN`〕／各設問 `choices` ≤20・`questions` ≤100・**合計 choice 上限なし**＝#65）。**README が挙げる数値上限（設問≤100・選択肢≤20/問）は今も正しい**ので直す対象は short の型・フィールド・例のみ（≤20 を「総数」と誤読しないこと＝per-question）。CLI の主用途は AI エージェントのクイズ量産で、消費側ワークスペース `../mazuoboeru-quizzes/`（`CLAUDE.md`・`check.mjs`）は 2026-06-27 に新契約へ更新済み＝README が唯一の取り残し。CLI に触る回に README も追従させる。
