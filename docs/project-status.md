...
| **Review List（#55・Phase 3 Slice 1。旧 Favorite を置換）** | ✅ **稼働（2026-06-19 実機確認）** | `GET /api/review-list` **401**（session 限定）。本番 D1 に `review_list` 実在・`favorite` は `0006` で DROP 済み（お試しデータ破棄・[ADR-0008](adr/0008-review-list-manual-pool.md)）。旧 `/api/favorites` は撤去＝SPA フォールバック 200。設問単位の手動プール（Drill/算入は Slice 2） |
