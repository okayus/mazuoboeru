...
>
> **最終更新: 2026-06-21**（更新したら日付と §「いま動いているもの」を直す）
...
- **短答（short）設問形式を実装・ローカル検証済み（2026-06-21・[ADR-0012](adr/0012-short-answer-grading-normalization.md)）＝コミット/本番反映はこれから**: 自由入力でタイプする一問一答（例「…各 namespace へのポインタをまとめた構造体は？」→「nsproxy」）。採点は **機械的正規化（NFKC＋trim＋連続空白畳み＋小文字化を入力と許容解の両辺へ）＋許容解リストへの完全一致**（あいまい一致なし。`worker/domain/short-answer.ts`）。許容解は `question.answer`（nullable JSON `{"accept":[生文字列,...]}`・生保存・`accept[0]` を正準解表示）、cloze は将来 `{"blanks":[...]}` へ拡張。`gradeQuestion`/`decideAnswer`・送信 zod・応答 DTO を `question.type` で**判別共用体化**（[ADR-0011](adr/0011-hono-rpc-typed-contract.md)）、`answer` は公開射影に非露出（[ADR-0010](adr/0010-server-side-grading-rationale.md)・client バンドルに採点コード非混入を grep 確認）。許容解と学習者入力は**プレーンテキスト**（[ADR-0004](adr/0004-ugc-markdown-rendering.md)）。公開ゲートは「許容解≥1」。作成 UI（テキスト入力・許容解 textarea）・挑戦・ドリル対応。**migration 0008 が判明した最大の罠**: `question` の CHECK 変更（rebuild）は D1/miniflare とも FK 強制 ON で `DROP TABLE question` が子（choice/review_list=CASCADE, attempt_answer/review_answer=NO ACTION）を壊す。`foreign_keys=OFF` は D1 が無視・`defer_foreign_keys` は文跨ぎで効かない（実証）→ **子4テーブルを新 question へ repoint してから drop→rename**＋**choice/review_list を CASCADE→NO ACTION 降格**（choice 削除はアプリ側へ。[ADR-0012] Addendum・[data-model](data-model.md)）。**検証: tsc(web/e2e)・lint・fmt・unit 106・build・`pnpm db:migrate`（miniflare）で 0008 適用＋`foreign_key_check` clean＋行数保全＋`type='short'` 挿入可を確認**。**remote 適用は host で backup＋行数チェック必須・Relay-Merge は付けない**（スキル `cloudflare-d1-drizzle-migration`）。CONTEXT.md に Short Answer／Accepted Answer／Answer Normalization を追加。
