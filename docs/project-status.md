...
- **その先の実装候補（次々セッション以降）**: ①Phase 2 残〔人気/ランキング・作者ページ・追加設問形式 `short`（実装・ローカル検証済＝ADR-0012・本番反映前）→`boolean`→`cloze`・Passkey・通報の Discord 通知・CLI npm 配信〕／②Phase 3 残〔タグ別習熟度（`answer` の蓄積を実効タグで集計）〕／③ツールチェイン段2〔dev/build の Rolldown 化＝wrangler 3→4＋@cloudflare/vite-plugin 1.x＋`vp build`/`vp dev`＋ratelimit 正式形移行。段1（lint/fmt/CI）は #63 反映済み＝[ADR-0009](adr/0009-vite-plus-toolchain.md)〕／④Phase 1 残〔投稿 per-user レート制限＝一般公開前・当面 allowlist で代替〕／⑤Attempt の引退〔**実装完了・ローカル全 green・本番反映待ち（未マージ・未デプロイ）**＝案B・[ADR-0013](adr/0013-retire-attempt-unify-answers.md)。ブランチ `claude/retire-attempt-unify-answers`・3スライス（answer 統合＝migration 0009 で review_answer リネーム／「挑戦」をクイズ単位 Drill 化＝Challenge 撤去／attempt DROP＝migration 0010＋ダッシュボードのクイズ別軸）。tsc・unit 91・lint・fmt・build・e2e 6/6・db:migrate 0009+0010＋SQL スモーク（attempt 消滅・FK clean）すべて green。**本番反映は host が migration 0010〔attempt/attempt_answer の破壊的 DROP〕を本番エクスポートで dry-run＋backup してからマージ**（merge＝Workers Builds が migrate→deploy 自動実行）＝memory `attempt-to-become-stateless-future`〕。
...
- **Attempt の引退（ステートレス化の最終形）— 設計確定（2026-06-24 grill・[ADR-0013](adr/0013-retire-attempt-unify-answers.md)）＋実装完了（同日・ローカル全 green・本番反映待ち）**: 案B＝[[Attempt]] エンティティを引退し、回答を単一フラット `answer(id, user_id, question_id, is_correct, answered_at)` に統合（旧 `review_answer` をリネーム＝migration 0009＋旧 `attempt_answer` を移行＝0010・既存データ保全）。「挑戦」は UI 動詞として残し＝クイズ単位 [[Drill]]（Review List／クイズ単位の両プール＋設問順シャッフル）に吸収。`response` 保存なし・由来列なし。CONTEXT.md（Challenge/Answer/Drill/Streak/Review List）・ADR-0013・[ADR-0006](adr/0006-dashboard-aggregation-semantics.md) addendum・data-model.md 改訂すべて完了。実装は `claude/retire-attempt-unify-answers`（3スライス＝answer 統合／クイズ単位 Drill 置換＝Challenge 撤去／attempt DROP＋ダッシュボードのクイズ別軸／退行ゼロ順序＝代替を先に出してから旧経路撤去）。**未マージ・未デプロイ**＝本番反映は host が migration 0010（破壊的 DROP）を本番エクスポートで dry-run＋backup 後にマージ（人間 merge・`Relay-Merge` は付けない）。auto-memory `attempt-to-become-stateless-future`。
