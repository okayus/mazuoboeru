...
>
> **最終更新: 2026-06-20**（更新したら日付と §「いま動いているもの」を直す）
...
- **その先の実装候補（次々セッション以降）**: ①Phase 2 残〔人気/ランキング・作者ページ・追加設問形式 `boolean`→`short`→`cloze`・Passkey・通報の Discord 通知・CLI npm 配信〕／②Phase 3 残〔タグ別習熟度（Drill/Attempt の蓄積を実効タグで集計）〕／③ツールチェイン段2〔dev/build の Rolldown 化＝wrangler 3→4＋@cloudflare/vite-plugin 1.x＋`vp build`/`vp dev`＋ratelimit 正式形移行。段1（lint/fmt/CI）は #63 反映済み＝[ADR-0009](adr/0009-vite-plus-toolchain.md)〕／④Phase 1 残〔投稿 per-user レート制限＝一般公開前・当面 allowlist で代替〕／⑤将来〔Attempt ステートレス化＝memory `attempt-to-become-stateless-future`・要 ADR〕。
...
- **【未修正バグ】クイズ作成 `POST /api/quizzes` が大きいクイズで 500（D1 の bound パラメータ上限 100）**: `createDraftQuiz()` → `contentStatements()`（`worker/db/quiz-queries.ts`）が **全 question を1本・全 choice を1本の multi-row INSERT** にまとめるため、D1 の「1クエリあたり bound パラメータ ≤100」を超えると SQLite が拒否し、route が catch せず **Hono 既定の 500（`text/plain` "Internal Server Error"・`apiError` JSON ではない）** になる。choice 行が 5 params/行で先に律速＝**総 choice >20（≒設問 >5）で破綻**（question は 6 params/行で ≤16設問）。実測（2026-06-20・本番）: 設問5＝20 choices＝**100 params → 201**、設問6＝25 choices＝**125 params → 500** で閾値は厳密に100。auth・scope・`mcq_multi`・tags はいずれも無関係（個別に 201 を確認済み）。既存の公開3本と CLI README の1問例は上限内だったため顕在化していなかった。**クライアント側の回避は不可**（PATCH も `replaceDraftContent`→同経路）。**直し**: `contentStatements()` の INSERT を ≤100 params にチャンク分割（choice ≤20行/文・question ≤16行/文、同一 `d.batch()` に積めば atomic 維持）→ create と PATCH が同時に直る＋単体テスト。**暫定回避（2026-06-20 実施）**: 22問の Linux 教材（リポ外 `~/indie-development/quiz.md`）を ≤20 choices の「Linux基礎①〜⑤」5本に分割し CLI `mzo` 経由で公開済み。
