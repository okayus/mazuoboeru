...
- **次の実装候補（次セッションは status→候補方針 2-3→grill の順で。未着手）**: ①Phase 2 残〔人気/ランキング・作者ページ・追加設問形式 `boolean`→`short`→`cloze`・Passkey・通報の Discord 通知・CLI npm 配信〕／②Phase 3 残〔タグ別習熟度（Drill/Attempt の蓄積を実効タグで集計）〕／③ツールチェイン〔lint/fmt スクリプト・CI 配線・dev/build の Rolldown 移行＝[ADR-0009](adr/0009-vite-plus-toolchain.md)・vite-plugin＋wrangler v4 連動〕／④Phase 1 残〔投稿 per-user レート制限＝一般公開前・当面 allowlist で代替〕／⑤将来〔Attempt ステートレス化＝memory `attempt-to-become-stateless-future`・要 ADR〕。
